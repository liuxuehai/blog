...
Redis 的源码阅读主线不是把数据结构逐个背下来，而是追踪“一条命令如何穿过事件循环、对象表示、持久化和复制”。本册重点回答：

1. **命令为什么能在单线程中保持高吞吐？** 事件循环如何串行执行命令，又把哪些工作交给后台线程或子进程。
2. **同一种逻辑数据为什么有多种编码？** SDS、listpack、quicklist、跳表和 dict 分别在什么规模与操作下切换。
3. **渐进式 rehash 解决了什么停顿？** 哈希表扩容如何分摊迁移成本，迁移期间读写访问哪张表。
4. **数据如何在重启后恢复，又如何复制给副本？** RDB、AOF、PSYNC 分别提供什么时间点和故障语义。
5. **过期和淘汰有什么区别？** 一个是键的逻辑生命周期，一个是内存压力下的容量策略。

读完整册后，应当能把“单线程”解释成命令执行边界，而不是误解成 Redis 内部所有工作都只有一个线程。
