...
## 先给答案：etcd 的核心不是一个简单的 KV server，而是一个由 Raft 排序、WAL 持久化、MVCC …

etcd 的核心不是一个简单的 KV server，而是一个由 Raft 排序、WAL 持久化、MVCC 状态机和事件派生器组成的复制状态机。 正文沿“分层 -> 启动链 -> 三条状态边界”展开：先确认入口和状态归属，再跟踪控制流或数据流的推进，最后落到对外可观察的结果。

主要失效边界集中在“committed index: 多数派确认的日志位置 applied index: 本节点已执行到的日志位置 MVCC revision: KV 状态机的逻辑版本 ```”这些场景。它们破坏的是容量、顺序、并发或生命周期前提；排查时应先确认状态是否仍由正确对象持有，再核对推进条件和清理路径。
