...
XXL-JOB 的核心不是 Cron 表达式，而是把一次任务拆成“计划、路由、执行、回调、重试和留痕”六个阶段。本册围绕一次调度实例回答：

1. **谁决定任务现在应该触发？** 调度中心扫描近期开启任务，把近秒触发放入时间轮。
2. **有多个执行器时交给谁？** 注册表给出候选节点，路由策略结合任务语义选择目标。
3. **同一任务尚未结束又被触发怎么办？** `JobThread` 与阻塞策略定义串行、丢弃或替换语义。
4. **失败之后为什么不能只“再试一次”？** 重试会创建新的调度压力，非幂等任务还可能产生重复副作用。
5. **执行发生在远端，调度中心如何知道结果？** 执行器保存日志并回调结果，admin 负责完成状态、告警和后续重试。
6. **分片广播和负载均衡有什么区别？** 前者要求多个节点共同覆盖数据，后者通常只选择一个节点执行。
