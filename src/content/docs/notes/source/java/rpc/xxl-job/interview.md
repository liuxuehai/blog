...
XXL-JOB 用数据库行锁串行化“谁来推进下一次触发时间”，锁内完成扫描和状态更新，锁外再由时间轮、触发线程池和执行器完成真正执行。调度、排队、执行、回调因此是几个相互衔接而非一次提交完成的阶段。

这意味着系统提供的是可恢复的最终状态机，不是端到端恰好一次：网络超时可能造成触发结果不确定，执行器队列和阻塞策略决定并发边界，回调失败还要靠重试收敛。业务任务仍需以幂等和可重入承接这些边界。
