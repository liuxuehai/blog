...
## 先给答案：Jedis 面试重点是连接状态、协议边界和 Cluster 重定向，而不是背命令 API

Jedis 面试重点是连接状态、协议边界和 Cluster 重定向，而不是背命令 API。 正文沿“高频题 -> 高难追问 -> 场景题”展开：先确认入口和状态归属，再跟踪控制流或数据流的推进，最后落到对外可观察的结果。

主要失效边界集中在“线上池耗尽怎么查、集群偶发 CROSSSLOT 怎么查”这些场景。它们破坏的是容量、顺序、并发或生命周期前提；排查时应先确认状态是否仍由正确对象持有，再核对推进条件和清理路径。
