...
第一轮解决“48 册是否完整落盘”，第二轮解决“读者能否先抓住问题与结论，再进入源码证据”。第二轮不增加仓库和篇数，分成“分册最低线”和“全篇完成线”两层验收，不能把前者记成后者：

1. 每册 `index.md` 增加“本册问题地图”，用 4~6 个问题串起该项目的状态、主流程、取舍与故障边界。
2. 每册至少选择一篇最能代表核心机制的页面，增加“先给答案”，先明确机制解决的矛盾、正常状态变化和主要失效边界。
3. **分册最低线**：总览有“本册问题地图”，且至少一篇代表性正文有“先给答案”；该状态只表示该册已开始形成问题—答案路径。
4. **全篇完成线**：该册除 `index.md` 外的每篇正文均有针对本篇机制的“先给答案”，并同步审阅日期。只有达到这一条，才可记为“第二轮全篇完成”。
5. 新增内容必须与正文已有源码坐标、图、边界与踩坑、可迁移知识一致，不能引入正文无法证明的新结论。
6. 修改后同步 `updatedDate`、`lastReviewed`，并通过构建和内部链接检查。

48 册全部达到“全篇完成线”后，第二轮才算整体完成。后续维护再进入第三类工作：按阅读反馈修正文、随上游版本复核坐标、对重点仓库做深度提升。
...
| 第二轮结构与统计 | `pnpm run check:source-reading` | 问题地图、答案区块或审阅日期结构错误 → 校验失败 |
| 第二轮最终验收 | `pnpm run check:source-reading:strict` | 任一正文尚未补齐“先给答案” → 校验失败 |
