...
第二轮不增加页面数量，而是在现有正文前补齐“问题 -> 答案”的阅读入口。两类区块都放在 `<!-- more -->` 之后、原有第一个二级标题之前。

`index.md` 必须有“本册问题地图”，用 4~6 个真实问题串起本册内容：

```markdown
## 本册问题地图

1. 请求或任务从哪里进入，主流程经过哪些状态？
2. 哪个对象拥有线程、连接、缓存或事务状态？
3. 为了吞吐、延迟或一致性做了什么取舍？
4. 故障时最先失效的边界在哪里，如何定位？
```

问题必须能落到本册具体页面，不能只把目录标题改写成问句。

除 `index.md` 外，每篇正文必须有且仅有一个“先给答案”，用两段完成预告：

```markdown
## 先给答案：<本篇机制的一句话结论>

第一段回答核心矛盾和正常状态变化：谁拥有状态，数据或控制流按什么顺序推进，机制为什么成立。

第二段回答主要失效边界：哪些前提被破坏后会出错，最常见的性能、并发、一致性或生命周期风险是什么。
```

- 标题和正文必须针对本篇机制，禁止跨页面复制同一套通用文案。
- “先给答案”只能总结正文已有证据；新增判断必须补充源码坐标、图、测试或明确标注为推断。
- 修改正文后同步 `updatedDate` 与 `lastReviewed`，两者使用同一个实际审阅日期。
- 分册最低线与全篇完成线以 [PLAN.md](./PLAN.md) §9.0 为准；只有每篇正文都满足上述要求，才算第二轮全篇完成。
...
1. Channel 为什么固定归属一个 EventLoop，这换来了什么？
2. 入站和出站事件如何沿 Pipeline 传播？
3. ByteBuf 的所有权何时转移，谁负责释放？
4. EventLoop 被阻塞或引用计数失配时如何定位？
...
第一段说明架构的核心矛盾、对象职责和两条主流程。

第二段说明线程归属、事件顺序和资源生命周期的主要失效边界。
...
第一段说明该机制解决的问题、关键对象和正常状态变化。

第二段说明容量、并发、回收或配置前提被破坏后的故障边界。
...
第一段把本册高频问题收束成一条可复述的机制主线。

第二段指出场景题最常落到的资源、顺序和生命周期边界。
...
- [ ] `index.md` 有 4~6 个具体问题组成的「本册问题地图」
- [ ] `index.md` 的「本册目录」列全了所有子页，链接可达
- [ ] 除 `index.md` 外，每篇正文有且仅有一个机制化的「先给答案」
- [ ] 修改页的 `updatedDate` 与 `lastReviewed` 已同步为实际审阅日期
...
- [ ] `pnpm run check:source-reading` 输出的册数、正文数与 [PROGRESS.md](./PROGRESS.md) 一致
- [ ] 第二轮最终验收时，`pnpm run check:source-reading:strict` 通过
